...
// Resolver eventoId por senha (usado pelo candidato na tela inicial)
app.post('/evento/por-senha', (req, res) => {
  const { senha } = req.body;
  if(!senha) return res.status(400).json({ erro: 'Senha obrigatória' });
  const { eventos } = lerDB();
  const evento = eventos.find(e => e.senha === senha);
  if(!evento) return res.status(404).json({ erro: 'Senha de evento inválida' });
  res.json({ eventoId: evento.id, nome: evento.nome });
});
...
// Excluir vídeo — apenas o próprio cliente pode excluir seus arquivos
app.delete('/cliente/videos', authCliente, async (req, res) => {
  const { key } = req.body;
  if (!key) return res.status(400).json({ erro: 'key obrigatória' });
  // Garante que o cliente só apaga arquivos do próprio clienteId
  if (!key.startsWith(`${req.cliente.clienteId}/`)) {
